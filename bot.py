...
aliexpress = AliexpressApi('514130',
...
def start_handler(message):
    ...
    subscribe_button = types.InlineKeyboardButton(
        "❤️ اشترك في القناة", url="https://t.me/Red_Store0")
    ...
    if message.chat.id == ADMIN_ID:
        try:
            ...
            download_button = types.InlineKeyboardButton(
                "📥 تحميل السجل", callback_data="download_again")
            keyboard.add(download_button)
            bot.send_message(message.chat.id,
                             "يمكنك تحميل السجل الكامل من الزر أدناه:",
                             reply_markup=keyboard)
...
def get_product_info(link, chat_id):
    try:
        ...
        if product_info:
            ...
            limited_offer_price = getattr(product, 'limited_offer_price',
                                          'غير متوفر')
            other_discount_price = getattr(product, 'other_discount_price',
                                           'غير متوفر')
            coins_discount_percentage = getattr(product,
                                                'coins_discount_percentage',
                                                'غير متوفر')
            store_name = getattr(product, 'store_name', 'غير متوفر')
            store_positive_rate = getattr(product, 'store_positive_rate',
                                          'غير متوفر')
            shipping_provider_name = getattr(product, 'shipping_provider_name',
                                             'غير متوفر')
